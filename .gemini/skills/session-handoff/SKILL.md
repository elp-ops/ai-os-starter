...
description: Use when the user says "session handoff", "wrap up session", "hand off", "handoff summary", or wants a structured end-of-session summary before clearing context. Produces a handoff covering decisions, shipped changes, key files, running state, verification steps, deferrals, and open questions so a fresh agent can continue seamlessly.
...
2. **Pull state from these sources (in order):**
   ...
   - Memory files written or updated (`memory/`).
...
4. **Produce the output in chat first.** Do not update memory from this skill.
5. **Then write the same content to `context/handoffs/<project-name>.md`**, where `<project-name>` is the argument passed to the skill (e.g., `/session-handoff healthbot` → `context/handoffs/healthbot.md`). If no argument is given, ask the user to provide one before writing. Overwrite the file every time -- it should always reflect the most recent handoff for that project.
...
1. **Chat output first, then write the file.** Produce the full handoff in chat, then write the identical content to `context/handoffs/<project-name>.md`. Never update memory from this skill.
...
3. **Absolute paths always**, inside the handoff file itself. The next agent may have a different working directory.
4. **If a plan file drove the session, name it first** in "Key files" so the next agent reads it before anything else.
5. **No hype, no padding.** Terse and concrete -- paths, commands, shell IDs, decisions. Match the tone of a seasoned engineer handing off at end-of-shift.
6. **Background process IDs are critical.** If any `run_in_background` shells were started, their IDs must appear in "Running state" with the kill command -- the next agent cannot find them otherwise.

## Anti-patterns -- do not do these

- Summarising the last 3 turns and calling it a handoff.
- Listing files by relative path inside the handoff content.
