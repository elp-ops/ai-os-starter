# Getting Started (Gemini Edition)
...
**Without this kit:** a blank Gemini CLI window. Every session starts from zero. No memory of what you told it yesterday. No way to pick up a project without re-explaining it. No structured way to even think through a plan before building it, let alone catch it drifting off course.
...
## Before you start: check what it costs
...
Go to [google.com/account](https://myaccount.google.com) and sign in with a Google account (or create one if you don't have one). Then check Google's current Gemini CLI access page to confirm which plan or licence you need, see "Before you start" above. Gemini CLI won't run without the right access in place.
...
Gemini CLI runs from a terminal (a text-based command window) rather than as a VS Code extension you click to install. VS Code has a built-in terminal, so you don't need to open a separate app.

**On a Mac:**
1. In VS Code, open the terminal: menu bar → Terminal → New Terminal.
2. If you don't already have [Homebrew](https://brew.sh) installed, install it first by following the instructions on that site.
3. In the terminal, type `brew install gemini-cli` and press enter. Wait for it to finish.
4. Type `gemini` and press enter to start it. Sign in with the Google account you used in Step 1.

**On Windows:**
1. In VS Code, open the terminal: menu bar → Terminal → New Terminal.
2. If you don't already have Node.js installed, install it first from [nodejs.org](https://nodejs.org) (the terminal is where Gemini CLI itself runs, but installing it needs Node.js, a separate program, in place first).
3. In the terminal, type `npm install -g @google/gemini-cli` and press enter. Wait for it to finish.
4. Type `gemini` and press enter to start it. Sign in with the Google account you used in Step 1.
...
1. In the VS Code terminal, inside the folder you just opened, type `gemini` and press enter to start Gemini CLI there.
...
**Connect a free Notion plan.** Good if you want a visual, organised home for your notes and documents that you can browse and search on its own, outside of Gemini CLI entirely. If you pick this, just ask your assistant to walk you through connecting Notion, it's a few minutes and it'll handle the setup with you.

**Run everything directly through Gemini CLI, as local files in this repo.** Simpler. No new account to set up. Works fine if nobody else needs to see or use the data.
...
Once you've decided, tell your assistant, and ask it to write your choice into `.gemini/rules/data-home.md` so it stops guessing.

## What's inside

A quick tour of what you just cloned:

- **`GEMINI.md`** — the instructions your assistant reads every single time it starts talking with you. This is where its name, personality, and how it should behave all live, including its always-on rules, folded directly into this one file since Gemini CLI only auto-loads a file literally named `GEMINI.md`, not an arbitrary folder.
...
- **`.gemini/skills/`** — specific things you can ask your assistant to *do*, not just ask about, like `/onboard`, `/audit`, and `/level-up`.
...
**Typing `gemini` in the terminal says "command not found."** The install in Step 3 either didn't finish or didn't put Gemini CLI somewhere your terminal can find it. Close the terminal, open a new one (Terminal → New Terminal), and try again. If it still fails, re-run the install command from Step 3.

**You typed `/onboard` and nothing happened, or it says the command isn't found.** Make sure the terminal is open inside the actual `ai-os-starter` *folder*, not somewhere else, before you type `gemini`. Gemini CLI needs to be started from the right folder to see its commands.

**You're signed in but Gemini CLI won't respond, or keeps asking you to sign in again.** Double check your account actually has current Gemini CLI access, see "Before you start" above. This is the most common snag right now given how recently Google changed who gets access.
