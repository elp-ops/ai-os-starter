...
**Without this kit:** a blank Claude Code window. Every session starts from zero. No memory of what you told it yesterday. No way to pick up a project without re-explaining it. No structured way to even think through a plan before building it, let alone catch it drifting off course.
...
Claude Code needs a paid Anthropic plan to actually run. A free account alone won't work. You'll need either a Claude Pro or Max subscription (through claude.ai, the simple option most people should use) or API billing set up (through console.anthropic.com, a more technical, pay-as-you-go option). Sort this out before you start installing anything, so you're not surprised by a paywall halfway through.
...
Go to [claude.ai](https://claude.ai) and sign up. Then make sure you actually have a paid plan active: either a Pro or Max subscription on claude.ai, or API billing set up at [console.anthropic.com](https://console.anthropic.com). Claude Code won't run without one.
...
2. Type "Claude Code" into the search box.
3. Click Install on the extension published by Anthropic.
4. A new Claude icon will appear in the sidebar. Click it and sign in with the account you made in Step 1.
...
1. Click the Claude icon in the sidebar to open the Claude Code panel.
...
**Connect a free Notion plan.** Good if you want a visual, organised home for your notes and documents that you can browse and search on its own, outside of Claude entirely. If you pick this, just ask your assistant to walk you through connecting Notion, it's a few minutes and it'll handle the setup with you.

**Run everything directly through Claude, as local files in this repo.** Simpler. No new account to set up. Works fine if nobody else needs to see or use the data.
...
Once you've decided, tell your assistant, and ask it to write your choice into `.claude/rules/data-home.md` so it stops guessing.

## What's inside

A quick tour of what you just cloned:

- **`CLAUDE.md`** — the instructions your assistant reads every single time it starts talking with you. This is where its name, personality, and how it should behave all live.
...
- **`.claude/skills/`** — specific things you can ask your assistant to *do*, not just ask about, like `/onboard`, `/audit`, and `/level-up`.
- **`.claude/rules/`** — always-on behaviour: how it talks to you, how carefully it spends effort and money, what it should never do without asking first.
...
**The Claude icon doesn't show up after installing the extension.** Quit VS Code completely and reopen it.

**You typed `/onboard` and nothing happened, or it says the command isn't found.** Make sure you opened the actual `ai-os-starter` *folder* in VS Code (File → Open Folder), not just a single file. The Claude Code panel needs to be pointed at the right folder to see its commands.

**You're signed in but Claude won't respond, or keeps asking you to sign in again.** Double check your paid plan is actually active — log into claude.ai and check Settings → Billing.
