...
3. **Delegate token-heavy steps** -- If the skill involves heavy searching, verbose API responses, or large output, delegate that step to a sub-agent. The skill instructs. The sub-agent does the work. Only the result returns to main context. Include the exact prompt to pass in the skill instructions.
...
2. For each prompt, run it twice:
   ...
   - **Without skill**: Execute the same prompt using only Claude's default behaviour, no skill instructions
...
- "Without skill" does not mean Claude is bad -- it means the skill may not be adding value.
...
3. For each phrase, assess: would the current description cause Claude to load this skill? (Yes / No / Uncertain)
4. Identify the gaps:
