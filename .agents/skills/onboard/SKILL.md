...
3. Write the answers into:
   ...
   - the Communication Style section of `AGENTS.md`
   - All `{{Your Name}}` placeholders throughout `AGENTS.md`, plus the `{{Filled by /onboard...}}` block under "Who {{Your Name}} is"
   - `# {{Ops Name — filled by /onboard. Default: {{Your Name}}'s Operating System}}` (line 1) and `{{Assistant Intro — filled by /onboard. Default: "You are {{Your Name}}'s assistant and thought partner inside this operating system. Your job: help them think clearly, decide faster, and get real work shipped, not just answer questions."}}` (line 3) in `AGENTS.md`:
