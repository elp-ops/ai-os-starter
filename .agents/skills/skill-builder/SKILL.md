...
Guides the creation and optimization of Codex skills using confirmed best practices. Use this whenever:
...
A skill is a reusable set of instructions that tells Codex how to handle a specific task. Skills live in `.agents/skills/[skill-name]/SKILL.md` inside your project. Codex loads a skill automatically when your request matches its `description`, or you can invoke one manually: run `/skills` to browse and pick one by hand, or type `$skill-name` to mention a specific skill directly (for example, `$skill-creator`).
...
**Round 1: Goal & Name**
*Why this matters: A clear goal prevents scope creep. The name becomes the skill's invocation name, so it needs to be memorable and specific.*
...
- Should it be user-only (invoked manually via `/skills` or `$skill-name`), Codex-auto-invocable, or both?
...
- For each step: does Codex do it directly, or does part of it get delegated (e.g. to a script)?
...
**Goal:** [one sentence]
**Trigger:** [natural language phrases], or `/skills` / `$name` to invoke manually
...
- **Task skills** (most common) give step-by-step instructions for a specific action. Invoked by name or natural language. Examples: generate a report, summarize a PR, deploy code.
...
- `description` -- Written as: "Use when someone asks to [action], [action], or [action]." Explain clearly when the skill should, and should not, trigger.
...
Your project's `AGENTS.md` file is where Codex loads project-wide instructions every conversation. After creating a skill, add a one-line entry to its skills list so you (and your team) know what's available:

- Skill name and how to invoke it
- Trigger phrases
...
2. **Direct invocation** -- Run `/skills` and select it, or type `$skill-name` to mention it directly, then give it test input
...
Input: [what you would say to trigger the skill, or the argument passed]
...
2. For each prompt, run it twice:
   ...
   - **Without skill**: Execute the same prompt using only Codex's default behaviour, no skill instructions
...
- "Without skill" does not mean Codex is bad -- it means the skill may not be adding value.
...
3. For each phrase, assess: would the current description cause Codex to load this skill? (Yes / No / Uncertain)
4. Identify the gaps:
